...
for i in urltxt:
    ...
    windows = tk.Tk()
    windows.title("爱套图爬虫")
    windows.geometry('725x900')  ## 规定窗口大小500*500像素
    windows.resizable(False, False)  ## 规定窗口不可缩放
    text1 = tk.Text(windows,width=100,height=1) #text宽度，有几行（行高）
    text1.grid(row=0,column=1,padx=10,pady=10)  #text所在行，所在列
    text2 = tk.Text(windows,width=100,height=1) #text宽度，有几行（行高）
    text2.grid(row=1,column=1,padx=10,pady=10)  #text所在行，所在列
    ...
    if(title != ''):        #检查图片名称是否为空
        ...
        if(flag == 1 and len(os.listdir(imgpath + title.strip().replace('?',''))) >= int(imgmun)):
            ...
            text1.delete(0.0,tk.END)    #删除旧消息
            text1.insert(tk.INSERT,title + "    张数:" + imgmax) #写入新消息
            text1.update()  #更新text信息
            text2.delete(0.0,tk.END)    #删除旧消息
            text2.insert(tk.INSERT,imgurl) #写入新消息
            text2.update()  #更新text信息
            filenametk = ('D:\\1\\1.jpg')    #图片路径
            print(filenametk)
            imtk=Image.open(filenametk)     #加载图片
            w,h=imtk.size
            print(w)
            print(h)
            imtk = resize(w,h,w_box,h_box,imtk)
            imgtk=ImageTk.PhotoImage(imtk)  #载入图片
            imLabel=tk.Label(windows,image=imgtk)   #载入图片到窗口
            imLabel.grid(row=2, column=1,padx=1,pady=1)   #图片所在行，所在列
            windows.after(360,windows.destroy)
            windows.mainloop()
            time.sleep(1)
    ...
    for n in range(int(hrefmin),int(imgmun)):    #从图片的初值开始循环，初值+图片的数量+1为循环的次数
        ...
        text2.insert(tk.INSERT,imgurl) #写入新消息
        text2.update()  #更新text信息
        filenametk = imgpath + title + "\\" + imgname    #图片路径
        print("***" + filenametk)
        imtk=Image.open(filenametk)     #加载图片
        w,h=imtk.size
        print(w)
        ...
        windows.update()    #窗口更新
        windows.after(60)
    windows.after(60,windows.destroy)
    windows.mainloop()